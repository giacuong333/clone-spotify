...
class PeekHoursView(APIView):
    ...
    def get(self, request):
        ...
        sorted_results = sorted(
            results, key=lambda x: x['play_count'], reverse=True)
...
class ExportPDFView(APIView):
    ...
    def get(self, request):
        ...
        report_type = request.query_params.get(
            'type', 'songs')  # songs, genres, peak_hours
        time_range = request.query_params.get('range', '30')  # Số ngày
        days = int(time_range)
        ...
        if report_type == 'songs':
            ...
            elements.append(
                Paragraph("Top Songs by Play Count", styles['Heading1']))
            ...
            data = [['Rank', 'Genre', 'Total Plays',
                     'Song Count', 'Average Plays per Song']]
            for i, genre in enumerate(response_data['results']):
                avg_plays = round(
                    genre['play_count'] / genre['song_count'], 2) if genre['song_count'] > 0 else 0
                data.append([
                    i+1,
                    genre['name'],
                    genre['play_count'],
            ...
            elements.append(
                Paragraph("Peak Hours Analysis", styles['Heading1']))
            elements.append(Spacer(1, 0.1*inch))
