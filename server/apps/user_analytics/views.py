...
from .serializers import (
    ...
    UserUploadedSongsSerializer,
    UserGenreStatSerializer,
)
...
class RecentlyPlayedView(APIView):
    ...
    def get(self, request):
        ...
        listened_history = ListenedAt.objects(user=request.user).order_by(
            "-listened_at"
        )
        ...
        for history in listened_history:
            ...
            if song_id not in song_ids:
                ...
                unique_songs.append(
                    {"song": history.song, "last_listened_at": history.listened_at}
                )
        ...
        # Serialize dữ liệu
        serializer = RecentlyPlayedSerializer(
            unique_songs, context={"request": request}, many=True
        )
...
class UserUploadStatsView(APIView):
    ...
    def get(self, request):
        ...
        time_period = request.query_params.get(
            "period", "all"
        )  # all, week, month, year
        ...
        if sort_by == "play_count":
            results = sorted(
                results, key=lambda x: x["play_count"], reverse=reverse_sort
            )
        elif sort_by == "download_count":
            results = sorted(
                results, key=lambda x: x["download_count"], reverse=reverse_sort
            )
        elif sort_by == "date":
            # Sắp xếp theo ngày tạo của bài hát
            results = sorted(
                results, key=lambda x: x["song"].created_at, reverse=reverse_sort
            )

        # Serialize dữ liệu
        serializer = UserUploadedSongsSerializer(
...
class UserGenreStatsView(APIView):
    ...
    def get(self, request):
        ...
        # Get user's listen history within the time range
        listen_history = ListenedAt.objects(
            user=request.user, listened_at__gte=start_date
        )

        # Count listens by genre
        genre_counts = Counter()
        genre_song_counts = {}  # Count distinct songs in each genre
        genre_names = {}  # Store genre names
        song_genre_map = {}  # Map songs to genres to avoid duplicate counting

        for listen in listen_history:
            song = listen.song
            song_id = str(song.id)

            # Check and get song genres
            if hasattr(song, "genre") and song.genre:
                # Handle song.genre being either a single genre or a list of genres
                genres = song.genre
                if not isinstance(genres, list):
                    genres = [genres]

                for genre in genres:
                    if hasattr(genre, "id"):  # Check if the genre has an id attribute
                        ...
                        # Count distinct songs (no duplicates)
                        if (song_id, genre_id) not in song_genre_map:
                            song_genre_map[(song_id, genre_id)] = True
            ...
            results.append(
                {
                    ...
                    "percentage": (
                        round((play_count / total_plays) * 100, 2)
                        if total_plays > 0
                        else 0
                    ),
                }
